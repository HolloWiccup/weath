...
function createTabDetails(weatherCity){
    ...
    temperature.textContent = `Температура: ${weatherCity.temperature}`;
    let feels_like = document.createElement('li');
    feels_like.textContent = `Ощущается: ${weatherCity.feels_like}`;
...
function createCardForecast(element){
    ...
    temperature.textContent = `Температура: ${Math.round(element.main.temp)}`;
