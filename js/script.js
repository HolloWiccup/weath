import { UI_ELEMENT, HEART, SERVER_URL, API_KEY } from "./constants.js";
...
function сityHandler(event){
    ...
    if(target.className === WEATHER_CLASS.ADD_CITY){
        ...
        target.src = storage.checkCity(city) ?
...
function createWeather(city){
    getFetch(city, SERVER_URL.WEATHER)
        ...
        .then(result => {
            if(result.cod >= 300){
                throw new Error('Не удалось получить Forecast')
            }
...
function getDiffTime(timezone, hours){
    let number = (24 - 3 + hours + timezone / 3600) % 24;
...
function fillInfoWindow(){    
    if(!Object.values(weatherCity).length) 
        return;
    
    UI_ELEMENT.INFO_WINDOW.replaceChildren();
    let tab;
    let activeTab = UI_ELEMENT.BUTTONS.querySelector('.-active').textContent;
    if(activeTab === 'Now')
        tab = createTabNow(weatherCity);
    if(activeTab === 'Details')
        tab = tab = createTabDetails(weatherCity);
    if(activeTab === 'Forecast')
        tab = createTabForecast(weatherCityWithArr);
